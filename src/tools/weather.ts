...
interface OpenMeteoResponse {
  latitude: number; longitude: number; timezone: string; elevation?: number;
...
function mmToIn(mm: number | null): string {
  if (mm === null || mm === undefined) return "N/A";
    ...
    try {
      ...
      const output = formatOutput({ latitude, longitude, timezone, forecast: daily }, () => {
        const rows = dates.map((date, i) => [
          date,
          celsiusToF(daily["temperature_2m_max"]?.[i] as number | null),
          celsiusToF(daily["temperature_2m_min"]?.[i] as number | null),
          mmToIn(daily["precipitation_sum"]?.[i] as number | null),
...
Args: latitude, longitude, start_date (YYYY-MM-DD), end_date (YYYY-MM-DD), timezone, response_format`,
    ...
    try {
      ...
      const daily = data.daily ?? {};
      const dates = (daily["time"] as string[] | undefined) ?? [];
      if (dates.length === 0) return { content: [{ type: "text", text: `No data found for ${start_date}→${end_date}. Earliest available: 1940-01-01.` }] };
      const display = dates.slice(0, 30);
      const maxT = (daily["temperature_2m_max"] as (number|null)[]) ?? [];
      const minT = (daily["temperature_2m_min"] as (number|null)[]) ?? [];
      const prec = (daily["precipitation_sum"] as (number|null)[]) ?? [];
