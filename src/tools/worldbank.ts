...
import type { WorldBankDataPoint } from "../types.js";
...
interface WBApiMeta { total?: number; page?: number; }
...
async function fetchWorldBankData(countryCode: string, indicator: string, startYear: number, endYear: number): Promise<WorldBankDataPoint[]> {
  ...
  const raw = await fetchJson<[WBApiMeta, WBDataItem[]] | WBApiMeta[]>(url);
  if (!Array.isArray(raw) || raw.length < 2) return [];
  const items = (raw[1] as WBDataItem[] | undefined) ?? [];
  return items
...
Args:
  ...
  - response_format: "markdown" or "json"`,
    ...
    try {
      ...
      if (data.length === 0) return { content: [{ type: "text", text: `No data found for "${indicator}" in "${country_code}" (${start_year}–${end_year}). Check the indicator code at data.worldbank.org/indicator.` }] };
