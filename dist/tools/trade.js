...
Args: reporter, partner (default "world"), year (2000-2023), flow ("all","X","M"), commodity_code, limit, response_format`,
        ...
        try {
            ...
            if (!raw.data?.length)
                return { content: [{ type: "text", text: `No trade data for ${reporter}↔${partner} in ${year}. Try year 2021, or use "TOTAL" for commodity_code.` }] };
            const flows = raw.data.slice(0, limit).map(d => ({
