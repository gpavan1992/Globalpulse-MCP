...
export function registerCountriesTools(server) {
...
Args: country (name, ISO alpha-2 e.g. "IN", or alpha-3 e.g. "IND"), response_format`,
        ...
        try {
            ...
            const rawList = await fetchJson(endpoint);
            if (!rawList?.length)
                return { content: [{ type: "text", text: `Country "${country}" not found. Try full name, ISO alpha-2 (e.g. "IN"), or alpha-3 (e.g. "IND").` }] };
...
Args: region, sort_by ("population","area","name"), limit, response_format`,
        ...
        try {
            let rawList = [];
            try {
                rawList = await fetchJson(`${API_ENDPOINTS.REST_COUNTRIES}/region/${encodeURIComponent(region)}`);
            }
            catch {
                rawList = await fetchJson(`${API_ENDPOINTS.REST_COUNTRIES}/subregion/${encodeURIComponent(region)}`);
            }
            if (!rawList?.length)
                return { content: [{ type: "text", text: `Region "${region}" not found. Valid: Africa, Americas, Asia, Europe, Oceania, or subregions like "South Asia".` }] };
            let countries = rawList.map(parse);
            if (sort_by === "population")
                countries.sort((a, b) => b.population - a.population);
            else if (sort_by === "area")
                countries.sort((a, b) => b.area_km2 - a.area_km2);
            else
