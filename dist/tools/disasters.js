...
export function registerDisasterTools(server) {
...
Args: event_type ("all","EQ","TC","FL","VO","DR","WF"), alert_level ("all","Green","Orange","Red"), limit, response_format`,
        ...
        try {
            ...
            if (events.length === 0)
                return { content: [{ type: "text", text: `No active disasters matching type="${event_type}", alert="${alert_level}".` }] };
...
Args: country (e.g. "India", "Philippines"), event_type, limit, response_format`,
        ...
        try {
            ...
            if (events.length === 0)
                return { content: [{ type: "text", text: `No disaster records for "${country}".` }] };
            const output = formatOutput(events, () => {
