...
export function registerAviationTools(server) {
...
India box: min_lat=8, max_lat=37, min_lon=68, max_lon=97.5 | Europe: min_lat=36, max_lat=71, min_lon=-10, max_lon=25`,
        ...
        try {
            ...
            if (!data.states?.length)
                return { content: [{ type: "text", text: "No aircraft found in this region." }] };
            let flights = data.states.map(parseState);
            if (on_ground_only)
                flights = flights.filter(f => f.on_ground);
    ...
    server.registerTool("globalpulse_aviation_get_flights_by_country", {
        ...
        try {
            const data = await fetchJson(`${API_ENDPOINTS.OPENSKY}/states/all`);
            if (!data.states)
                return { content: [{ type: "text", text: "No data from OpenSky." }] };
            let flights = data.states.map(parseState).filter(f => f.origin_country.toLowerCase() === country.toLowerCase() && !f.on_ground);
            const total = flights.length;
            if (total === 0)
                return { content: [{ type: "text", text: `No airborne flights from "${country}". Check spelling matches OpenSky format (e.g. "United States" not "USA").` }] };
            flights = flights.slice(0, limit);
